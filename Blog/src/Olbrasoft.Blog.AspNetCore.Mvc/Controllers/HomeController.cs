...
public class HomeController : BlogController
{
    ...
    {
        var categories = await _categoryService.CategoriesAsync(token);
        var tags = await _tagService.TagsAsync(token);

        return new RightColumnViewModel
        {
            Categories = categories.Any() ? categories.SplitToTwo() : Tuple.Create(Enumerable.Empty<CategorySmallDto>(), Enumerable.Empty<CategorySmallDto>()),
            Tags = tags.Any() ? tags.SplitToTwo() : Tuple.Create(Enumerable.Empty<TagSmallDto>(), Enumerable.Empty<TagSmallDto>())
        };
    }
